...
Steps:
1. Filter and keep select features from non-transcriptomic data (FACS data and cytokine levels)
...
6. For each dose group, build separate modules (one for each feature) adjusting for study site, presence of Pf infection at first vaccination, and pre-vax CSP-specific IgG
7. For each dose group, combine significant features in a unifying multi-variable model.
8. Convert pvalues to FDR and use it to keep only significant correlations
...
```{r save data as matrix for ML analysis}
...
### BTM-level network plots (Figure 5A of manuscript)
