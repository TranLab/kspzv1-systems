...
temp <- tempfile(fileext = ".rds")
dl <- drive_download(
  as_id("1NR0SKRw37ZUTnKfvdedCPWoyOv3aDECU"), path = temp, overwrite = TRUE)
...
camera_hm_hibtm_monaco_plot <- plotDat_Camera %>%
  ...
  ggplot(., aes(x = study, y = module, fill = Direction)) +
...
```{r plot camera bubbleplot hi btms, fig.align='center', fig.width=12, fig.height=11, echo=FALSE, message=FALSE, warning=FALSE}
#addSmallLegend(camera_hibtms_plot)
ggpubr::ggarrange(camera_hm_hibtm_monaco_plot, camera_lowbtm_plot, camera_bloodgen3_plot,
                  ncol = 3, widths = c(0.78,1,0.78),common.legend = TRUE)
```
