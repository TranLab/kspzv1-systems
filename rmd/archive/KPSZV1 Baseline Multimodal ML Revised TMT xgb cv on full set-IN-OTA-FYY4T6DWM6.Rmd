...
1. Perform training using xgboost with hyperparameter tuning with 4-fold cross-validation using random search strategy in 2500 total runs on full dataset.
2. Downselection of features:


3. Again using only the training sets, 3 to 7 features were subsampled from the features appearing in at least 20 times in 1000 runs (>=2%), and the training was again run using these subsampled, downselected features. The same approach noted in #2 above was used except that performance metric was AUC.
...
This was set to "full" based on reviewer's request. We cannot go back to splitting at this point.
...
### Evaluate most frequently appearing features
...
Bypass read-in in of feature list if running from start
...
### Read-in high-dose PfSPZ baseline models

```{r load in highpdse baseline models}
load("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/full_high_dose_4fold_cv_2500_runs/all_folds_data_seed_1943_runs_2500_19_Aug_2023_full.RData")

#read in model performance
ml_res <- readxl::read_excel("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/full_high_dose_4fold_cv_2500_runs/cv_summarized_res_all_folds_seed_1943_runs_2500_htune_iters_500_19_Aug_2023_full.xlsx") %>%
  arrange(desc(mean_cv_accuracy))

```

## Predict placebo group using high-dose PfSPZ model

Determine if features from the highest high-dose PfSPZ xgb models, as determined by mean CV accuracy > 80%, can predict the opposite class in the placebo dataset

```{r confusion matrix and importance}
no_models_to_test <- nrow(ml_res[ml_res$mean_cv_accuracy>.80,]) #select only the high-dose PfSPZ models with accuracy >80%.
test_labels_for_confusion_matrix <- factor(ifelse(test_labels==1, "protected", "not protected"))
xgbDMat_test_dat <- xgbpred <- confusion_mat <- overall_accuracy <- c()
for(i in ml_res$run[1:no_models_to_test]){
  xgbDMat_test_dat[[i]] <- xgb.DMatrix(data = new_test[, fold1[[i]]$downselected_xgb_results$feature_names], label = placebo_dummy_label, nthread = 8)
  xgbpred[[i]]$pred_res <- predict(fold1[[i]]$downselected_xgb_results, xgbDMat_test_dat[[i]])
  xgbpred[[i]]$pred_res_dichotomous  <- ifelse(xgbpred[[i]]$pred_res  > 0.5,1,0)
  #remember that we are going with the original high-dose designation where infected=0, protected=1.
  xgbpred[[i]]$pred_res_labeled   <- factor(ifelse(xgbpred[[i]]$pred_res_dichotomous==1, "protected", "not protected")) 
  confusion_mat[[i]] <- caret::confusionMatrix(xgbpred[[i]]$pred_res_labeled, test_labels_for_confusion_matrix)
  overall_accuracy[[i]] <- confusion_mat[[i]]$overall
}
overall_accuracy_df <- bind_rows(overall_accuracy, .id = "run") %>%
  right_join(., ml_res[1:no_models_to_test, ] %>%
              dplyr::select(run, Features),
            by = "run") %>%
  arrange(desc(Accuracy))
```

```{r save placebo confusion matrix results, include=FALSE, echo=FALSE, }
resdir <- "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/"
saveRDS(confusion_mat, file = paste0(resdir,
                                     "full_high_dose_test_placebo/",
                                                    "confusion_mat_high_dose_on_placebo",
                                                    gsub("\\:","-", format(Sys.time(),"%a-%b-%d-%X-%Y")),".rds"))
writexl::write_xlsx(overall_accuracy_df,
                    paste0("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/JCI Resubmission April 2023/Supplementary Tables JCI resubmission/",
                           "Table SX accuracy high-dose PfSPZ models on placebo data.xlsx"))
```

### Highlight Run 211

This model performed the best on the placebo data.
...
smooth <- TRUE
no_runs_to_plot <- 2
best_run <- best_combo <- response <- predictor <- roc_dat <- auc <- c()
for(j in c(overall_accuracy_df[1:no_runs_to_plot,]$run)){
  best_run[[j]] <- overall_accuracy_df[overall_accuracy_df$run==j,] %>%
    group_by(run) %>%
    slice_max(Accuracy) %>%
    distinct(Features) %>%
    pull(var = run)
  best_combo[[j]] <- overall_accuracy_df[overall_accuracy_df$run==j,] %>%
    group_by(run) %>%
    slice_max(Accuracy) %>%
    distinct(Features) %>%
    pull(var = Features)
  #calculate predictive accuracy
  response[[j]] <- as.numeric(test_labels_for_confusion_matrix)-1 #has to be numeric, protected = 1, not protected = 0
  predictor[[j]] <- xgbpred[[j]]$pred_res
  roc_dat[[j]] <- pROC::roc(response = response[[j]],
                            predictor = predictor[[j]],
                            smooth = smooth) 
  auc[[j]] <- roc_dat[[j]]$auc %>% as.numeric(.) %>% signif(.,digits=3)
  }
auc_df <- bind_rows(auc, .id = "run")
...
ggplotConfusionMatrix(confusion_mat$`run 2222`)
```
