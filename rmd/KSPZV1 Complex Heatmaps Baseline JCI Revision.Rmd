---
title: "KSPZV1 Complex Heatmaps Baseline, JCI revision"
author: "Tuan M. Tran"
date: "09/22/2023"
...
```{r load cpm eset local, message=FALSE, warning=FALSE, eval=FALSE, echo=FALSE}
#local
...
```{r load cpm eset gdrive, message=FALSE, warning=FALSE}
#from google drive
...
```{r load add additional baseline data, message=FALSE, warning=FALSE}
#CBC data
...
```{r filter by mads, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
## Filtering features
#This is not used in paper. MADS set to 100% (include all genes after low-expression filter)
...
```{r add btm data to features, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
## Add BTM data to features
#This is used for exploratory purposes. Not in manuscript.
source("https://github.com/TranLab/kspzv1-systems/blob/main/helper/Convert%20High%20Level%20BTM%20List%20into%20Long%20Tibble.R?raw=TRUE")
fData(x) <- fData(x) %>%
...
if(ClusterCols == TRUE){
    ...
    hm <- Heatmap(exprs(x),
                  ...
                  #row_split = paste("gene cluster ",pa.row$clustering, sep = "_"), #removed row split
...
```{r save heatmap png, fig.width= 10, fig.height=7, fig.align="center", echo=FALSE, eval=FALSE}
#set dimensions
myheight <- 10
mywidth <- 16
CairoPNG(paste0(figdir, "Fig_1C_JCI_Heatmap", "_height_", myheight, "_width_", mywidth, "_seed_", myseed,".png"), height = 480, width = 480*2.5)
print(hm)
dev.off()
```

```{r save heatmap pdf, fig.width= 10, fig.height=7, fig.align="center", echo=FALSE, eval=FALSE}
#set dimensions
myheight <- 10
mywidth <- 16
cairo_pdf(paste0(figdir, "Fig_1C_JCI_Heatmap", "_height_", myheight, "_width_", mywidth, "_seed_", myseed,".pdf"), height = myheight, width = mywidth)
print(hm)
dev.off()
```

```{r draw to extract clustering info columns, message=FALSE, warning=FALSE, fig.width= 14, fig.height=6, fig.align='center'}
hm <- draw(hm, heatmap_legend_side="top", annotation_legend_side="right",
           legend_grouping = "original")
```

```{r save heatmap data df, include=FALSE, echo=FALSE, eval = FALSE}
plot_dat_path <- "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/JCI Resubmission April 2023/Data for all Plots JCI Resubmission/"

#annotation df
writexl::write_xlsx(annot.df, path = paste0(plot_dat_path, "Figure 1B Baseline Heatmap Annotation Dataframe.xlsx"))

#heatmap data
ordered_hm_dat <- data.frame(exprs(x)[unlist(row_order(hm)), unlist(column_order(hm))]) %>%
  rownames_to_column(var = "annotation_or_ensembl_ID") %>%
  mutate(across(everything(), as.character))

ordered_annot_dat <- data.frame(t(annot.df[unlist(column_order(hm)),]))  %>%
  rownames_to_column(var = "annotation_or_ensembl_ID") %>%
  mutate(across(everything(), as.character))

ordered_combined_dat <- bind_rows(ordered_annot_dat, ordered_hm_dat)

exprs(x) %>%
  as.data.frame() %>%
  rownames_to_column(var = "ensembl_id") %>%
  writexl::write_xlsx(., path = paste0(plot_dat_path, "Figure 1B Baseline Heatmap Matrix Data.xlsx"))

writexl::write_xlsx(data.frame(unlist(column_order(hm))), path = paste0(plot_dat_path, "Figure 1B Baseline Heatmap Column Order.xlsx"))

writexl::write_xlsx(data.frame(unlist(row_order(hm))), path = paste0(plot_dat_path, "Figure 1B Baseline Heatmap Row Order.xlsx"))

writexl::write_xlsx(data.frame(unlist(row_order(hm))), path = paste0(plot_dat_path, "Figure 1B Baseline Heatmap Row Order.xlsx"))

writexl::write_xlsx(ordered_combined_dat, path = paste0(plot_dat_path, "Figure 1B Baseline Ordered Heatmap and Columna Annotations.xlsx"))
```
...
```{r make stats plots based on clusters reviwer response, message=FALSE, warning=FALSE}
...
```{r make stats plots based on clusters reviwer response additional, message=FALSE, warning=FALSE}
...
## Print column annotation bar and box plots (Figure S2)
...
cairo_pdf(filename = paste0(figdir, "Fig S2 Revision ", pval_label, " ", adjust_p,".pdf"), height = 8, width = 8)
print(ggarrange(
  Outcome.counts,
  tte_dat_boxplot,
  csp_baseline_dat_boxplot,
...
### Compare distribution between P and NP between SC1 Placebo vs SC1 1.8x10^6 PfSPZ

```{r make sc1 specific barplot, echo=TRUE, message=FALSE, warning=FALSE, fig.align='center', fig.width=5, fig.height=5}
...
```{r plot make sc1 specific barplot, echo=FALSE, fig.align='center', fig.height=4, fig.width=4}
Outcome.counts
```


```{r save make sc1 specific barplot, echo=FALSE, eval=FALSE}
cairo_pdf(filename = paste0(figdir, "Fig S5B SC1 sub Revision.pdf"), height = 1.25, width = 1)
Outcome.counts
dev.off()
```

```{r extract clustering information rows, message=FALSE, warning=FALSE, include=FALSE, eval=FALSE, echo=FALSE}
...
```{r merge with fData, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
## Merge with fData(x)

#This is for evaluating gene clusters, which is is not in the manuscript.
...
```{r plot zscore of genes in cluster by sampleCluster, message=FALSE, warning=FALSE, fig.height=7.85, fig.width=2.25, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r plot zscore of genes in cluster by sampleCluster print to knit, message=FALSE, warning=FALSE, fig.height=12, fig.width=5, fig.align='center', include=FALSE, echo=FALSE, eval=FALSE}
## Print error plot of Gene Expression (Figure 1D)
print(myErrorPlot)
```


```{r plot zscore of genes in cluster by sampleCluster print to PDF, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r apply fgsea, message=FALSE, warning=FALSE, fig.width = 12, fig.heigth = 12, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r GSEA plot for baseline CPM Figure Panel D, message=FALSE, warning=FALSE, fig.height = 4, fig.width=3.5, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r plot GSEA enrichment, message=FALSE, warning=FALSE, fig.width=5, fig.height=5, fig.align='center', include=FALSE, echo=FALSE, eval=FALSE}
addSmallLegend(myGSEAClusterPlot)
```

```{r print GSEA enrichment, message=FALSE, warning=FALSE, fig.width=6, fig.height=4, echo = FALSE, eval = FALSE, include=FALSE}
