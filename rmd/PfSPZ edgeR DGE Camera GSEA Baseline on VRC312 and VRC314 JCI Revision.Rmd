...
Run DGE, Camera, GSEA for baseline VRC 312 and VRC 314 datasets.
...
```{r read in files local, echo=FALSE, message=FALSE, warning=FALSE}
#local paths
...
design <- model.matrix(~0 + Gender + Group + Batch1 + Batch2 + Outcome) # dose adjusted; batch corrected; edgeR
...
#saveRDS(myCameraPlotDat_VRC312, paste0(datadir, "VRC312_Camera_PvNP_FDR5.rds"))
```
...
writexl::write_xlsx(myCameraPlotDat, paste0(datadir,"PfSPZ_Camera_VRC312", "_",
                          "P_v_S_",
                          my_VRC312_Group,"_",
                          myTIMEPOINT,"_",
                          "Pval",
...
Outcome_Timept <- factor(paste(Outcome,Timept,sep="_"))
design <- model.matrix(~0 + Group + Sex + Batch + Outcome) # GROUP and BATCH corrected
rownames(design) <- Subject
colnames(design) <- gsub(":", "_", colnames(design))
colnames(design) <- gsub("\\.", "_", colnames(design))
colnames(design) <- gsub(", ", "_", colnames(design))
...
#saveRDS(myCameraPlotDat_VRC314, paste0(datadir, "VRC314_Camera_PvNP_FDR5.rds"))
```
...
```{r load bspzv1 degs from local path, include=FALSE, eval=FALSE, echo=FALSE}
...
```{r load bspzv1 degs from googledrive, include=FALSE, eval=FALSE, echo=FALSE}
...
myCameraPlotDat_VRC312 <- read_rds("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/JCI Resubmission April 2023/Prelim Results JCI resubmission/VRC312_Camera_PvNP_FDR5.rds")

myCameraPlotDat_VRC314 <- read_rds("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/JCI Resubmission April 2023/Prelim Results JCI resubmission/VRC314_Camera_PvNP_FDR5.rds")
```

```{r combine VRC312 VRC314 BSPZV1 camera dat}
...
Cameratab_BSPZV1_VRC312_VRC314 <- bind_rows(myCameraPlotDat_BSPZV1, myCameraPlotDat_VRC312, myCameraPlotDat_VRC314)                                    
```

```{r write Cameratab_BSPZV1_VRC312_VRC314, eval=FALSE, include=FALSE, echo=FALSE}
Cameratab_BSPZV1_VRC312_VRC314 %>%
  writexl::write_xlsx(.,
                      paste0(datadir, "PfSPZ_Camera_PvS_BSPZV1_VRC312_VRC314_sex_adj.xlsx"))
```

### Visualize Camera data as bubble plot

Filter based on padj < 0.05.

Add small legend helper function

```{r helper fn make small legend, echo=TRUE}
...
```{r visualize camera data, message=FALSE, warning=FALSE}
#Red: enriched in protected/uninfected 
#Blue: enriched in not protected/infected

plotDat_Camera <- Cameratab_BSPZV1_VRC312_VRC314 %>%
  mutate(study = factor(study, levels = c("VRC312", "VRC314", "BSPZV1"))) %>%
...
camera_hm_hibtm_monaco_plot <- plotDat_Camera %>%
  filter(module_type %in% c("MSigDB_Hallmark_v7.4", "highBTMs", "MonacoModules")) %>%
  mutate(module_type = factor(module_type,
                              levels = c("highBTMs", "MonacoModules", "MSigDB_Hallmark_v7.4"),
                              labels = c("high BTMs", "Monaco", "Hallmark"))) %>%
...
```{r make vrc and bspzv1 study tables}
study_tab <- data.frame(matrix(c("VRC 312", "3a", "30,000 PfSPZ x 6", "3", "0",
                           "", "3b", "30,000 PfSPZ x 4", "8", "1",
                           "", "4a", "135,000 PfSPZ x 5", "0", "2",
                           "", "4b", "135,000 PfSPZ x 5", "0", "4",
                           "", "4c", "135,000 PfSPZ x 4", "3", "6",
                           "", "All", "", "14", "13",
                           "VRC 314", "1", "270,000 PfSPZ x 3", "1", "3",
                           "", "2", "270,000 PfSPZ x 4", "0", "5",
                           "", "4", "270,000 PfSPZ x 4", "5", "6",
                           "", "5", "900,000 PfSPZ x 3", "5", "9",
                           "", "All", "", "1", "23",
                          "BSPZV1", "2", "135,000 PfSPZ x 5", "7", "4", 
                          "", "3", "270,000 PfSPZ x 5", "7", "4",
                          "", "All", "", "14", "8"),
                          nrow = 14, byrow = T))
colnames(study_tab) <- c("Study", "Group","Regimen","Not Protected","Protected")
...
Only modules with FDR<5% are shown.

```{r plot camera bubbleplot hi btms, fig.align='center', fig.width=10, fig.height=11, echo=FALSE, message=FALSE, warning=FALSE}
#addSmallLegend(camera_hibtms_plot)
study_tab_plot <- ggpubr::ggtexttable(study_tab, rows = NULL, theme = ttheme("classic", base_size=8, padding = unit(c(1.5, 1.5), "mm")))
tab_hm_hibtm_monaco_plot <- ggpubr::ggarrange(study_tab_plot, camera_hm_hibtm_monaco_plot, nrow=2, heights = c(0.3,1), widths = c(0.5,1), common.legend = TRUE)
ggpubr::ggarrange(tab_hm_hibtm_monaco_plot, ggplot() + theme_void(), camera_lowbtm_plot, ggplot() + theme_void(), camera_bloodgen3_plot,
...
GSEA_BSPZV1_VRC312_VRC314 <- bind_rows(GSEAtab_VRC312,
                     GSEAtab_VRC314) %>%
  filter(module_type %in% c("highBTMs", "lowBTMs", "MonacoModules", "BloodGen3Module", "MSigDB_Hallmark_v7.4"))  %>%
  ...
  mutate(module_type = factor(module_type, levels = c("highBTMs", "MonacoModules", "lowBTMs", "MSigDB_Hallmark_v7.4", "BloodGen3Module"))) %>%
  droplevels() %>%
  bind_rows(.,GSEAtab_BSPZV1) %>%
  arrange(study, module_type, desc(neglogpadj))
```

```{r write GSEA_BSPZV1_VRC312_VRC314, eval=FALSE, include=FALSE, echo=FALSE}
GSEA_BSPZV1_VRC312_VRC314 %>%
  writexl::write_xlsx(.,
                      paste0(datadir, "PfSPZ_GSEA_PvS_BSZPV1_VRC312_VRC314_sex_adj.xlsx"))
```
...
TopPlot <- GSEA_BSPZV1_VRC312_VRC314 %>%
...
MidPlot <- GSEA_BSPZV1_VRC312_VRC314 %>%
  filter(`BH-adj p value (FDR)`<0.05) %>%
...
BottomPlot <- GSEA_BSPZV1_VRC312_VRC314 %>%
  filter(`BH-adj p value (FDR)`<0.05) %>%
  filter(module_type %in% c("BloodGen3Module"))  %>%
  ggplot(., aes(x = study, y = pathway)) +
  geom_point(aes(size=neglogpadj, fill = NES), alpha = 0.65, shape=21, stroke = 0.25) +
...
### Plot GSEA bubbleplots (Figures S4B of revised manuscript)

Red: enriched in protected/uninfected 
Blue: enriched in not protected/infected
Only modules with BH-adjusted p value (FDR) < 0.05 are shown.

```{r plot gsea bubbleplot top plat, fig.align='center', fig.width=5.75, fig.height=3.55, echo=FALSE, message=FALSE, warning=FALSE}
addSmallLegend(TopPlot)
```

```{r plot gsea bubbleplot mid plot, fig.align='center', fig.width=4.75, fig.height=12.75, echo=FALSE, message=FALSE, warning=FALSE}
addSmallLegend(MidPlot)
```

```{r plot gsea bubbleplot bottom plott, fig.align='center', fig.width = 4, fig.height = 9, echo=FALSE, message=FALSE, warning=FALSE}
addSmallLegend(BottomPlot)
```


```{r save gsea plots, echo=FALSE, eval=FALSE, include=FALSE}
pdf(file = paste0(figdir, "Fig_S4B_GSEA_highBTMs_Monaco_VRC312_VRC314_BSPZV1.pdf"), height = 3.75, width = 5.25)
addSmallLegend(TopPlot)
dev.off()

pdf(file = paste0(figdir, "Fig_S4BGSEA_lowBTMs_Hallmark_VRC312_VRC314_BSPZV1.pdf"), height = 10.25, width = 4.5)
addSmallLegend(MidPlot)
dev.off()

pdf(file = paste0(figdir, "Fig_S4B_GSEA_BloodGen3Modules_VRC312_VRC314_BSPZV1.pdf"), height = 9.75, width = 3.5)
addSmallLegend(BottomPlot)
dev.off()
```