---
title: "KSPZV1 delta 1.8x10^6 PfSPZ multimodal ML revised - xgboost nested feature selection and cv FULL DATASET"
author: "Tuan M. Tran"
...
complete_delta_eset <- read_rds(paste0(datadir, "KSPZV1 logCPM expression sets for visualization/PfSPZ_cpm_ExpressionSet_230x23768_allGroups_bothBatches_delta_rmBatchFX_06082021_TMT.rds"))
high_dose_features <- read_rds(paste0(datadir2,"highdose_PfSPZ_delta_correlation_ML_data_with_missing.rds"))
high_dose_delta_eset <- complete_delta_eset[,which(complete_delta_eset$PATID %in% rownames(high_dose_features))]
```

```{r impute missing, eval=FALSE, include=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
...
high_dose_features_class <- t(high_dose_features) %>%
  ...
  left_join(., pData(complete_delta_eset) %>%
...
high_dose_features_class <- high_dose_features_class[colnames(high_dose_delta_eset),]

if(all(rownames(high_dose_features_class) == colnames(high_dose_delta_eset))){
  print("good to go!")
} else {
...
# write_rds(high_dose_features_class, file = paste0(datadir2, "high_dose_delta_class_features_scaled_imputed.rds"))
# write_rds(t(high_dose_features), file = paste0(datadir2, "high_dose_delta_features_only_scaled_imputed.rds"))
```

```{r read high dose delta features local, include=FALSE, echo=FALSE, eval=FALSE}
high_dose_features_class <- read_rds(file = paste0(datadir2, "high_dose_delta_class_features_scaled_imputed.rds"))
```

```{r read high dose delta features googledrive, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r if starting from prior session, include=FALSE, eval=FALSE, echo=FALSE}
load(paste0("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/",
            "full_high_dose_delta_4fold_cv_486_runs/all_folds_data_seed_highdose_delta_seed_2991_runs_486_12_Oct_2023_full.RData"))
```
...
```{r load all folds data from local, include=FALSE, eval=FALSE, echo=FALSE}
#load(paste0("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/",
#            "full_high_dose_delta_4fold_cv_2500_runs/all_folds_data_seed_1943_runs_2500_19_Aug_2023_full.RData"))
```
...
```{r save results of crossvalidation, include=FALSE, eval=FALSE, echo=FALSE}
save(fold1, fold2, fold3, fold4,
       file = paste0("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/",
                     "full_high_dose_delta_4fold_cv_", length(fold4), "_runs/",
