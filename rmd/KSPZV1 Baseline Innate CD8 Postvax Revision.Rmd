...
Addresses review 2 question to more directly show that innate activation limits CD8+ T cell responses in humans.
...
## Method 1: Differential Gene Expression Analysis
...
```{r print GSEA detected vs not detected bubble plot, fig.align='center', fig.height=7, fig.width=9, fig.align='center', echo=FALSE}
...
cairo_pdf(filename = "/Volumes/GoogleDrive/My Drive/Manuscripts/KSPZV1 Manuscript/Med Revised Submission 2022/Figures/Revised Figure Panels/Fig 7X Detected vs not detected GSEA PfSPZ CD8 edgeR placebo highdose PfSPZ hiBTMs bbubbleplot.pdf", width = 5, height = 3)
print(addSmallLegend(myGSEAClusterPlot))
dev.off
```

```{r print GSEA detected vs not detected bar plot, fig.align='center', fig.height=7, fig.width=9, fig.align='center', echo=FALSE}
...
cairo_pdf(filename = "/Volumes/GoogleDrive/My Drive/Manuscripts/KSPZV1 Manuscript/Med Revised Submission 2022/Figures/Revised Figure Panels/Fig 7X Detected vs not detected GSEA PfSPZ CD8 edgeR placebo highdose PfSPZ hiBTMs barplot.pdf", width = 5.5, height = 3.5)
print(addSmallLegend(myGSEAClusterPlot))
dev.off
```
...
```{r combine facs with gene expression}
combo_dat <- pfspz_cd8 %>%
  left_join(., exprs(x) %>%
              t() %>%
              as.data.frame() %>%
...
```{r print GSEA plot, fig.align='center', fig.height=7, fig.width=9, fig.align='center', echo=FALSE}
addSmallLegend <- function(myPlot, pointSize = 6, textSize = 6, spaceLegend = 1) {
    myPlot +
        guides(shape = guide_legend(override.aes = list(size = pointSize)),
               color = guide_legend(override.aes = list(size = pointSize))) +
...
```{r plot, fig.align='center', fig.width=10, fig.height=6}
library(ggpubr)
...
```{r plot 2, fig.align='center', fig.width=10, fig.height=60}
library(ggpubr)
options(scipen=999) # no scientific notation
combo %>%
