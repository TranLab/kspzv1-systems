...
```{r plot gsea bubbleplot, fig.align='center', fig.width=4.5, fig.height=6, echo=TRUE, message=FALSE, warning=FALSE}
...
### Determine relationship of baseline MBC and plasmablast modules with baseline CSP-specific IgG

```{r readin data for MES and CSP Igg}
library(googledrive)
library(tidyverse)
...
baseline_eset <- baseline_eset[,baseline_eset$treat== "1.8 x 10^6 PfSPZ"]
...
IgG_dat <- pData(x) %>%
  ...
  dplyr::select(SAMPLEID, CSPAb, log2FC_CSPAb, treat) %>%
  full_join(., baseline_pas_monaco %>%
              t() %>%
              data.frame() %>%
...
```{r scatter plots}
PB_base <- IgG_dat %>%
  ggplot(., aes(x = Plasmablasts_monaco_baseline, y = log10(CSPAb+1))) +
  geom_point() +
  stat_cor() + 
...
MBC_base <- IgG_dat %>%
  ggplot(., aes(x = B_Memory_monaco_baseline, y = log10(CSPAb+1))) +
  geom_point() +
  stat_cor() + 
  stat_smooth(method = "lm") +
  theme_bw()
...
PB_LFC <- IgG_dat %>%
  ggplot(., aes(x = Plasmablasts_monaco_baseline, y = log2FC_CSPAb)) +
  geom_point() +
  stat_cor() + 
  stat_smooth(method = "lm") +
  theme_bw()
...
MBC_LFC <- IgG_dat %>%
  ggplot(., aes(x = B_Memory_monaco_baseline, y = log2FC_CSPAb)) +
  geom_point() +
  stat_cor() + 
  stat_smooth(method = "lm") +
  theme_bw()

ggarrange(PB_base,MBC_base, PB_LFC, MBC_LFC, ncol = 2)
```