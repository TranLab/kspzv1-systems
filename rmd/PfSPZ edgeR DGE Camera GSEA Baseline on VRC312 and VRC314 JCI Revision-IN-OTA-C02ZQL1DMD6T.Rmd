...
4. Use FDR<5% as final filter for visualization
5. Added CAMERA results to complement pre-ranked GSEA results. This was motivated by prior reviewer's comments that pre-ranked GSEA did not account for inter-gene correlations. Results here show that CAMERA results is similar to pre-ranked GSEA results.
...
study_tab_plot <- ggtexttable(study_tab,
                                      rows = NULL,
                                      theme = ttheme("classic", base_size=8, padding = unit(c(8, 2.5), "mm")))
...
TopPlot <- GSEA_KPSZV1_VRC312_VRC314_BPSZV1 %>%
  ...
  theme(axis.title.x = element_blank(),
    ...
    theme(plot.margin = unit(c(1, 0.3, 1, 0.3), "cm")) +
...
Bottom_Left_Plot <- GSEA_KPSZV1_VRC312_VRC314_BPSZV1 %>%
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.position = "right",
        axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
    theme(plot.margin = unit(c(1, 0.45, 5, 0.45), "cm")) +
...
Bottom_Middle_Plot <- GSEA_KPSZV1_VRC312_VRC314_BPSZV1 %>%
  ...
  theme(axis.title.x = element_blank(),
        ...
        legend.position = "right",
        axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
    theme(plot.margin = unit(c(1, 0.7, 15, 0.7), "cm")) +
facet_wrap(~module_type, scales = "free_y", nrow =2)
...
pdf(file = paste0(figdir, "Fig_S4B_GSEA_VRC312_VRC314_BSPZV1_FDR", fdr_threshold*100,".pdf"), height = 15, width = 12)
ggarrange(addSmallLegend(TopPlot), bottom_panels, common.legend = TRUE, nrow = 2, heights = c(1,3))
dev.off()
```
...
```{r combine VRC312 VRC314 BSPZV1 camera dat, message=FALSE, warning=FALSE}
...
```{r plot camera bubbleplot hi btms, fig.align='center', fig.width=10, fig.height=11, echo=TRUE, message=FALSE, warning=FALSE}
#addSmallLegend(camera_hibtms_plot)
ggpubr::ggarrange(camera_hm_hibtm_monaco_plot, ggplot() + theme_void(), camera_lowbtm_plot, ggplot() + theme_void(), camera_bloodgen3_plot,
