---
title: "KSPZV1 Complex Heatmaps Δ (postvax/baseline), JCI Revision"
author: "Tuan M. Tran"
...
```{r optimal clusters Elbow, echo=FALSE, eval=FALSE,message=FALSE, warning=FALSE, include=FALSE, eval=FALSE, echo=FALSE}
...
## Print heatmap (Figure 4A) and extract cluster information

```{r save heatmap, fig.width= 10, fig.height=7, fig.align="center", eval=FALSE, include=FALSE, echo=FALSE}
#set dimensions
# myheight <- 10
# mywidth <- 20
...
#annotation df
writexl::write_xlsx(annot.df, path = paste0(plot_dat_path, "Figure 4A Delta Heatmap Annotation Dataframe.xlsx"))
...
exprs(x) %>%
  ...
  writexl::write_xlsx(., path = paste0(plot_dat_path, "Figure 4A Delta Heatmap Matrix Data.xlsx"))

writexl::write_xlsx(data.frame(unlist(column_order(hm))), path = paste0(plot_dat_path, "Figure 4A Delta Heatmap Column Order.xlsx"))

writexl::write_xlsx(data.frame(unlist(row_order(hm))), path = paste0(plot_dat_path, "Figure 4A Delta Heatmap Row Order.xlsx"))

writexl::write_xlsx(data.frame(unlist(row_order(hm))), path = paste0(plot_dat_path, "Figure 4A Delta Heatmap Row Order.xlsx"))

writexl::write_xlsx(ordered_combined_dat, path = paste0(plot_dat_path, "Figure 4A Delta Ordered Heatmap and Column Annotations.xlsx"))
```
...
## Print column annotation bar and box plots (Figure S5)
...
```{r plot outcome counts sc2, fig.align='center',fig.height=4, fig.width=4}
Outcome.counts
```


```{r save make sc2 specific barplot, eval=FALSE, echo=FALSE, include = FALSE}
# cairo_pdf(filename = paste0(figdir, "Fig S5B SC2 sub Revision.pdf"), height = 1.25, width = 1)
# Outcome.counts
# dev.off()
```

```{r extract clustering information rows, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r figure 4C preprint, echo=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
knitr::kable(foo)
```
...
```{r GSEA plot for baseline CPM Figure Panel D, message=FALSE, warning=FALSE, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r plot GSEA enrichment delta, message=FALSE, warning=FALSE, fig.width=5, fig.height=5, fig.align='center', include=FALSE, echo=FALSE, eval=FALSE}
