...
Revised analyses to perform module-trait relationship as moderated t tests.
...
myBreaks <- c(seq(-1*max(abs(all_module_trait_hm_mat)), 0, length.out=ceiling(paletteLength/2) + 1), 
              seq(max(all_module_trait_hm_mat)/paletteLength, max(abs(all_module_trait_hm_mat)), length.out=floor(paletteLength/2)))
myColor <- colorRampPalette(c("dodgerblue", "white", "#b22222"))(length(myBreaks))

cairo_pdf(file = "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/JCI Resubmission April 2023/Figures JCI resubmission/Figure 4B Revised to WGCNA limma.pdf", width = 5, height = 4)
