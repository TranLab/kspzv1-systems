---
title: "KSPZV1 post-vax 1.8x10^6 PfSPZ multimodal ML revised - xgboost nested feature selection and cv on full dataset"
author: "Tuan M. Tran"
...
```{r libraries, message=FALSE, include=TRUE, echo=TRUE, eval=FALSE}
library(openxlsx)
library(xgboost)
library(dplyr)
library(tidyverse)
...
```{r set local paths, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r set group and timepoint, include=FALSE, echo=FALSE, eval=FALSE}
#Set group and timepoint
dose_group <- "highdose"
timepoint <- "postvax"
```

```{r read in full dataset and impute any missing data, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r impute missing, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r make highdose features, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r read high dose postvax features local, include=FALSE, echo=FALSE, eval=FALSE}
high_dose_features_class <- read_rds(file = paste0(datadir2, "high_dose_post_vax_class_features_scaled_imputed.rds"))
```

```{r read high dose postvax features googledrive, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r select options, include=TRUE, echo=TRUE, eval=FALSE}
train_on_split_or_full_set <- "full" #options: original_split, split, full
```

```{r split test and train, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r use mlr3 package set parameters and resampling strategy, message=FALSE, include=TRUE, echo=TRUE, eval=FALSE}
library(mlr)
library(mlr3)
...
```{r hyperparameter tuning to select best features loop set up and options, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r source function and make fold objects, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r if starting from prior session, include=FALSE, echo=FALSE, eval=FALSE}
...
```{r run fs and cv loop, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r load all folds data from local, include=FALSE, echo=FALSE, eval=FALSE}
load(paste0("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/",
            "full_high_dose_postvax_4fold_cv_2500_runs/all_folds_data_seed_1943_runs_2500_19_Aug_2023_full.RData"))
```

```{r combine results from all folds, include=TRUE, echo=TRUE, eval=FALSE}
...
```{r save results of crossvalidation, include=FALSE, echo=FALSE, eval=FALSE}
