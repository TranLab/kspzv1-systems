---
title: "KSPZV1 postvax high-dose PfSPZ multimodal ML- visualize xgboost ML results, JCI revision (Figure 6C)"
author: "Tuan M. Tran"
...
```{r ad high dose postvax features from local, eval=FALSE, include=FALSE, include=FALSE}
high_dose_features_class <- readRDS(paste0(datadir2, "high_dose_post_vax_class_features_scaled_imputed.rds"))
...
all_folds_2500_runs <- load(paste0(datadir,
                                  "full_high_dose_postvax_4fold_cv_2500_runs/",
                                  "all_folds_data_seed_highdose_postvax_seed_3443_runs_2500_11_Oct_2023_full.RData"))
per_run_res_2500_runs <- readxl::read_xlsx(paste0(datadir,
                                  "full_high_dose_postvax_4fold_cv_2500_runs/",
                                  "cv_results_all_runs_all_folds_highdose_postvax_seed_3443_runs_2500_htune_iters_500_11_Oct_2023_full.xlsx"))
summarized_res_2500_runs <- readxl::read_xlsx(paste0(datadir,
                                  "full_high_dose_postvax_4fold_cv_2500_runs/",
                                  "cv_summarized_res_all_folds_highdose_postvax_seed_3443_runs_2500_htune_iters_500_11_Oct_2023_full.xlsx"))
```
...
### Determine across-fold performance metrics (accuracy, AUC, Brier score, kappa, logless) select best performing high-dose PfSPZ post-vax models
...
writexl::write_xlsx(across_fold_perform_high_dose, paste0("/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/JCI Resubmission April 2023/Supplementary Tables JCI resubmission/","Table S6 across folds xgb performance highdose postvax.xlsx"))
```
...
writexl::write_xlsx(roc_plots$data, paste0(plot_dat_dir, "Fig 6C post-vax high-dose PfSPZ ROC plot.xlsx"))
```

### Plot confusion matrices for high-dose postvax models
...
```{r make confusion matrix plots highdose postvax, message=FALSE, warning=FALSE}
library(ggplot2)
library(scales)
...
foo <- bind_rows(my_plots[[1]]$data,
          ...
          my_plots[[4]]$data)
foo$model <- foo_ids
writexl::write_xlsx(foo, paste0(plot_dat_dir,"Fig 6C post-vax high-dose PfSPZ confusion matrices.xlsx"))
```
...
pdf(paste0(plotdir, "Figure 6C highdose postvax feature by pct appearance barplot AverageRank.pdf"),
...
writexl::write_xlsx(feature_importance_plot$data, paste0(plot_dat_dir,"Fig 6C post-vax high-dose PfSPZ feature importance barplot.xlsx"))
```
...
writexl::write_xlsx(foo, paste0(plot_dat_dir,"Fig 6C post-vax high-dose PfSPZ SHAP plot data.xlsx"))
```
...
ggsave(filename = paste0(plotdir, "Figure 5C highdose postvax AUC Confusion Matrix Shap plots for top 4 models JCI revision across folds AverageRank.pdf"), plot=figure_5C_plot, height=6, width=16, device = "pdf")
```
...
pdf(paste0(plotdir, "Figure S5D highdose postvax feature values by SHAP values by variable by model AverageRank top ", top_n,".pdf"), width = 13, height = 5)
shap_by_feature
dev.off()
```
...
save(xgb_res, train_features, file = paste0(
  "/Users/tuantran/Library/CloudStorage/OneDrive-IndianaUniversity/Manuscripts/KSPZV1 Manuscript/ML results Tuan/full_high_dose_postvax_4fold_cv_2500_runs/",
  "highdose_pfspz_postvax_top_4_models_by_AverageRank_xgb_res_for_shap_plots_v", my_number, ".RData"))
```

### Confirm differences in CD3+CD4+ T cells between NP and P in placebo group at baseline

```{r confirm differences in CD3+CD4+ cells, fig.align='center', fig.width=3.75, fig.height=6, message=FALSE, warning=FALSE}
library(ggpubr)
...
```{r save cd4 plot, eval=FALSE, include=FALSE, echo=FALSE}
ggsave(filename = paste0(plotdir, "Figure SX CD4 IgM MBC FACS NP vs P highdose PfSPZ postvax.pdf"),plot = CD4_IgM_plot, device = "pdf",
       units = "in", width = 3.6, height = 6)
```